...
API 서버(54.91.162.220:8080)와 같은 서버에 프론트엔드를 배포하는 방법입니다.
...
# 서버에 업로드 (로컬 터미널에서 실행)
scp -i capstone.pem -r dist/* ec2-user@54.91.162.220:/var/www/onit/
...
server {
    listen 80;
    server_name 54.91.162.220;  # 또는 실제 도메인
    root /var/www/onit;
    index index.html;
...
# 또는 브라우저에서 접속
# 프론트엔드: http://54.91.162.220
# API: http://54.91.162.220/api (Nginx가 자동으로 프록시)
```
