...
export default function VoiceCallTestPanel({
  ...
  const lastMuteRequestRef = useRef(null);
  ...
  useEffect(() => {
    if (!muteRequestKey) return;
    if (lastMuteRequestRef.current === muteRequestKey) return;
    lastMuteRequestRef.current = muteRequestKey;
    if (desiredMuted === isMuted) return;

    handleToggleMute();
  }, [muteRequestKey, desiredMuted, isMuted]);

  useEffect(() => {
    if (!isInCall || desiredMuted === isMuted) return;
    handleToggleMute();
  }, [isInCall, desiredMuted, isMuted]);
