...
const InfiniteCanvasPage = () => {
  ...
  const [voiceMuteRequestKey, setVoiceMuteRequestKey] = useState(null);
  const [voicePreferredMuted, setVoicePreferredMuted] = useState(false);
  ...
  const activeVoiceChannelMembers = activeVoiceSessionId
    ...
    : [];
  const voiceChannelMembersByChannel = Object.fromEntries(
    voiceSessionChannels.map((channel) => [
      channel.id,
      voiceChannelMembersBySession[String(channel.sessionId)] || []
    ])
  );
  ...
  const handleToggleVoiceMute = useCallback(() => {
    setVoicePreferredMuted((prev) => !prev);
    setVoiceMuteRequestKey(`mute:${Date.now()}`);
  }, []);
  ...
  return (
    <div className="infinite-canvas-page">
      ...
      <ChatPanel 
        ...
        externalVoiceParticipantsByChannel={voiceChannelMembersByChannel}
      ...
      {workspaceId && currentUserId && (
        <VoiceCallTestPanel
          ...
          muteRequestKey={voiceMuteRequestKey}
          desiredMuted={voicePreferredMuted}
          controllerOnly={!showVoiceTestPanel}
