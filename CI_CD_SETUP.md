...
- 서버에 rsync가 설치되어 있는지 확인:
  ```bash
  ssh -i capstone.pem ec2-user@100.55.36.73 "which rsync"
  ```
- 설치되어 있지 않으면:
  ```bash
  ssh -i capstone.pem ec2-user@100.55.36.73 "sudo dnf install -y rsync"
  ```
...
4. 서버에서 배포된 파일 확인:
   ```bash
   ssh -i capstone.pem ec2-user@100.55.36.73 "ls -la /var/www/onit/"
   ```
