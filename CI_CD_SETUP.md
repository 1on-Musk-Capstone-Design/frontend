...
- 서버에 rsync가 설치되어 있는지 확인:
  ```bash
  ssh -i capstone.pem ec2-user@43.201.225.38 "which rsync"
  ```
- 설치되어 있지 않으면:
  ```bash
  ssh -i capstone.pem ec2-user@43.201.225.38 "sudo dnf install -y rsync"
  ```
...
4. 서버에서 배포된 파일 확인:
   ```bash
   ssh -i capstone.pem ec2-user@43.201.225.38 "ls -la /var/www/onit/"
   ```
